...
public static void printBinaryTree_1(List<List<Node>> list, String pattern[][], int n){
		...
		int row = 0, column=0;
		for(int i=1; i<n; i++) {
			int nodesToPrint =  list.get(i-1).size();//(int) Math.pow(2, i-1); //2
			int initialSpace = (int) (Math.pow(2, n-i)-1);//3
...
//			}
			...
			for(int l=1; l<=nodesToPrint; l++) {//1-2
				pattern[row][column++] = list.get(i-1).get(l-1).getValue(); //
				
				if(l==nodesToPrint)
					break;
				
				column+=(Math.pow(2, n-i+1)-1);
